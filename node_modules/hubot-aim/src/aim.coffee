  ...
  run: ->
    ...
    aim.connect (err) ->
      if not err
        ...
        if process.env.HUBOT_AIM_ROOMS
          aim.joinChat(process.env.HUBOT_AIM_ROOMS, -> console.log)
    ...
    aim.on "chatmsg", (chatroom_name, sender, message) ->

      # Strip out HTML tags
      tmp = message.replace(/<(?:.|\n)*?>/gm, '')
      hubot_msg_trimmed = tmp.replace(/^[ \t]+/, '')

      console.log 'chatroom: ' + chatroom_name + ', sender: ' + sender + ', message: ' + message + ', trimmed message: ' + hubot_msg_trimmed
      self.receive new TextMessage(chatroom_name, hubot_msg_trimmed)
